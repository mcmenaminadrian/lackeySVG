...
class GraphCompFifo {
	...
	/**
	 * Graph the lifetime functions of both the FIFO and
	 * LRU methods of page caching
	 * @param thetaFIFOAveMap data about the FIFO cache
		...
		def width, def height, def gridMarks, def boostSize)
	{
		...
		def maxT
		def maxG
		def maxWSS = (thetaFIFOAveMap.values()).max()
		def maxLRU = (thetaLRUAveMap.values()).max()
		if (maxWSS > maxLRU) {
			thetas = thetaFIFOAveMap.keySet()
			gs = thetaFIFOAveMap.values()
		} else {
		...
		maxG = gs.max()
		maxT = thetas.max()
		def writer = new FileWriter ("CMPFIFOLRU${new Date().time.toString()}.svg")
		...
		svg.text(x:boostSize/4, y: height / 2,
			...
			"Denning's g(theta)")
		def strInst = "Average set size: red fifo, blue lru"
